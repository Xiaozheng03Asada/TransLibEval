...
output_folder = "CodeLlama-7b_python_sum"  # 生成的文本文件将保存在该文件夹
...
# 修改后的 API 调用函数
def analyze_code_with_llama(source_code, class_name, retries=5):
    ...
    for attempt in range(retries):
        try:
            # 使用 CodeLlama-7b-Instruct 模型进行调用
            resp = comp.do(
                model="CodeLlama-7b-Instruct",  # 使用 CodeLlama-7b-Instruct 模型
                prompt=prompt,
                temperature=0.01,
                top_p=0.95
            )
...
def process_python_file(python_file):
    ...
    summary = analyze_code_with_llama(source_code, class_name)
