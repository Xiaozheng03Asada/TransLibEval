...
# 定义输入文件夹路径和输出文件夹
input_folder = "qwmax_python_sum"  # 包含功能梗概的文件夹（针对 Java 文件的总结）
...
def process_summary_file(summary_file_path, target_language):
    ...
    base_name = os.path.basename(summary_file_path).replace(".txt",
                                                            f".{'java' if target_language == 'Java' else 'cpp'}")
    output_folder = output_folder_java if target_language == 'Java' else output_folder_cpp
