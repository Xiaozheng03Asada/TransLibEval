...
PY_DIR = Path("python")
OUTPUT_ROOT = Path("Meta-Llama-3-70B")  # ← 顶级输出目录同步更名
...
MODEL_NAME = "Meta-Llama-3-70B-Instruct"  # ✅ 目标模型更换处


def translate_code(prompt: str, retries: int = 3) -> str | None:
    """使用 CodeLlama‑7b‑Instruct 生成代码实现。"""
...
def process_function(base: str, target: str) -> None:
    ...
    ext = "java" if target == "java" else "cpp"
    ...
    py_code = py_path.read_text(encoding="utf-8", errors="ignore") if py_path.exists() else ""
