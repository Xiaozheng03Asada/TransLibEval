...
PY_DIR = Path("python")
OUTPUT_ROOT = Path("qwen-max-2025-01-25")
...
def translate_code(prompt: str, retries: int = 3) -> str | None:
    """
    使用 qwen-max-2025-01-25 模型进行代码翻译
    """
...
def make_prompt(base: str, target: str, so_answers: list[str], signature: dict, py_code: str) -> str:
    sig_json = json.dumps(signature, ensure_ascii=False)
    ref_impl = "\n".join(so_answers) if so_answers else py_code or "（无可用参考实现）"
    return (
        "You are a world‑class expert in code generation with deep mastery of translating "
        f"algorithmic Python class methods into {target} implementations.\n\n"
        "Below are the precise function signature details and either community‑sourced reference implementations "
        "or the original Python code as fallback. Your task is to generate clean, idiomatic, and fully functional "
...
def process_function(base: str, target: str):
    out_dir = OUTPUT_ROOT / f"py_to_{target}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ext = "cpp" if target == "cpp" else "java"
    ...
    if py_path.exists():
        py_code = py_path.read_text(encoding="utf-8", errors="ignore")

    prompt = make_prompt(base, target, so_ans, sig, py_code)
