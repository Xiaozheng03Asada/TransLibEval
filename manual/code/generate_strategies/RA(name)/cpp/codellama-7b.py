...
⚠️ 本版本已将大模型调用由 deepseek‑v3 调整为 CodeLlama‑7b‑Instruct，
   并改用 Qianfan 官方 SDK（qianfan.Completion）替代手写 HTTP 请求。
"""
...
def process_function(base: str, target: str) -> None:
    ...
    cpp_code = cpp_path.read_text(encoding="utf-8", errors="ignore") if cpp_path.exists() else ""
