...
# 定义输入文件夹路径和输出文件夹
input_folder = "qwmax_java_sum"  # 包含功能梗概的文件夹（针对 Java 文件的总结）
output_folder_root = "qwen-max"  # 顶级文件夹，用于存放生成的代码文件夹
...
def process_summary_file(summary_file_path, target_language):
    ...
    base_name = os.path.basename(summary_file_path).replace(".txt",
                                                            f".{'py' if target_language == 'Python' else 'cpp'}")
