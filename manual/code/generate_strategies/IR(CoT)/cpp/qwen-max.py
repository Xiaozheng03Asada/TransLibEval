...
# 定义输入文件夹路径和输出文件夹
input_folder = "qwmax_cpp_sum"  # 包含功能梗概的文件夹
output_folder_root = "qwen_max"  # 顶级文件夹，用于存放生成的代码文件夹
output_folder_python = "generated_python"  # 生成的 Python 文件将保存在该子文件夹
    ...
    for attempt in range(retries):
        try:
            response = client.chat.completions.create(
                model="qwen-max-2025-01-25",  # 使用 qwen_max 模型
...
def process_summary_file(summary_file_path, target_language):
    ...
    base_name = os.path.basename(summary_file_path).replace(".txt",
                                                            f".{'py' if target_language == 'Python' else target_language.lower()}")
    output_folder = output_folder_python if target_language == 'Python' else output_folder_java
