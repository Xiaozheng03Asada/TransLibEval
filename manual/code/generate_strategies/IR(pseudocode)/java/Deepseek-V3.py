...
# 定义输入文件夹路径和输出文件夹
input_folder = "Deepseek_java_sum"  # 包含功能梗概的文件夹
output_folder_root = "Deepseek-v3"  # 顶级文件夹，用于存放生成的代码文件夹
output_folder_py = "generated_python"  # 生成的 Python 文件将保存在该子文件夹
output_folder_cpp = "generated_cpp"  # 生成的 C++ 文件将保存在该子文件夹

# 创建输出文件夹（如果不存在），首先创建顶级文件夹
os.makedirs(output_folder_root, exist_ok=True)
os.makedirs(os.path.join(output_folder_root, output_folder_py), exist_ok=True)
...
def process_summary_file(summary_file_path, target_language):
    ...
    base_name = os.path.basename(summary_file_path).replace(".txt", f".{'py' if target_language == 'Python' else 'cpp'}")
    output_folder = output_folder_py if target_language == 'Python' else output_folder_cpp
    ...
    if generated_code:
        # 提取代码块
        extracted_code = extract_code_block(generated_code, target_language)
        if extracted_code:
            # 保存生成的代码到相应文件夹
