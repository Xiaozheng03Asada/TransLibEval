...
# 定义输入文件夹路径和输出文件夹
input_folder = "Meta-Llama-3-70B_python_sum"  # 包含功能梗概的文件夹
output_folder_root = "Llama3-70B"  # 顶级文件夹，用于存放生成的代码文件夹
...
# 调用 Qianfan API（Meta-Llama-3-70B）进行代码生成
def generate_code_with_meta_llama(summary, language, retries=5):
    ...
    for attempt in range(retries):
        try:
            # 调用 Qianfan API 使用 Meta-Llama-3-70B 模型
            response = comp.do(
...
def process_summary_file(summary_file_path, target_language):
    ...
    # 调用 Meta-Llama-3-70B API 生成目标语言代码
